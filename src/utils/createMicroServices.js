...
function main() {
  ...
  readline.question('Ingrese el nombre de la carpeta en donde instanciará el microservicio (captado, captador, comun, public):', (folder) => {
    if (fs.existsSync(route + '/' + folder)) {
      route = route + '/' + folder;

      // Second callback

      readline.question(`Ingrese el nombre del microservicio ( en ${path.basename(route)} ):`, (name) => {
...
// File/Folder manager for the microservice
